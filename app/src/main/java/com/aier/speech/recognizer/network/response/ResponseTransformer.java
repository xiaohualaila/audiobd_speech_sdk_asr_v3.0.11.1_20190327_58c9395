package com.aier.speech.recognizer.network.response;

import com.aier.speech.recognizer.network.Exception.ApiException;
import com.aier.speech.recognizer.network.Exception.CustomException;
...
public class ResponseTransformer {
    ...
    private static class ResponseFunction<T> implements Function<Response<T>, ObservableSource<T>> {
        ...
        public ObservableSource<T> apply(Response<T> tResponse) throws Exception {
            boolean success = tResponse.isSuccess();
            int code = tResponse.getErrcode();
            String message = tResponse.getErrmsg();
            if(success){
